...
const formations = [
  {
    date: "Depuis 2025",
    title: "Diplôme d'ingénieur en Design Industriel Durable",
    school: "EPF École d'Ingénieurs, Troyes",
    desc: "Formation d'ingénieur orientée conception, innovation produit, développement durable et approche design appliquée à l'industrie.",
  },
  {
    date: "Depuis 2024",
    title: "Cycle d'ingénieur en Génie Mécanique",
    school: "Institut International de Technologie, Sfax",
    desc: "Approfondissement en conception mécanique, industrialisation, calcul et environnement technique de production.",
  },
  {
    date: "Septembre 2021 - Juin 2024",
    title: "Licence appliquée en Génie Mécanique",
    school: "Institut Supérieur des Études Technologiques, Sfax",
    desc: "Base solide en génie mécanique, métrologie, fabrication, mise en plan, CAO et résolution de problèmes techniques.",
  },
];

const projects = [
  {
    title: "Conception mécanique & dimensionnement",
    description:
      "Analyse fonctionnelle, dimensionnement mécanique, simulation et validation expérimentale d'un système complet.",
    image: "/images/projet-1.jpg",
    tags: ["SolidWorks", "RDM 6", "Prototype", "Blender"],
    hasDetail: true,
  },
  {
    title: "Projet fil rouge",
    description:
      "Développement de nouveaux produits à partir de matériaux composites recyclés, avec prototypes et validation fonctionnelle.",
    image: "/images/projet-3.jpg",
    tags: ["Durabilité", "Composites", "Prototype"],
    hasDetail: false,
  },
];
...
const skillCategories = [
  {
    ...
    items: ["SolidWorks", "CATIA V5", "AutoCAD", "Python", "Arduino"],
  },
  {
    title: "Compétences mécaniques",
    items: [
...
const contacts = [
  {
    icon: "📞",
    label: "Téléphone",
    value: "+33 7 53 90 23 28",
    href: "tel:+33753902328",
  },
  {
    icon: "✉️",
    label: "Email",
    value: "oussama.rebai@epfedu.fr",
    href: "mailto:oussama.rebai@epfedu.fr",
  },
  {
    icon: "📄",
    label: "CV",
    value: "Télécharger mon CV",
    href: "/cv/Oussama_REBAI_CV.pdf",
    download: true,
  },
  {
    icon: "💼",
    ...
    href: "https://www.linkedin.com/in/oussama-rebai-80713b3b2/",
    external: true,
  },
];
...
function RevealTiltCard({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div
      ...
      className={`card ${visible ? "visible" : ""} ${className}`}
    >
      {children}
    </div>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      sections.forEach((s) => {
        ...
        if (rect.top <= 150 && rect.bottom >= 150) current = s.id;
      });
  ...
  const { ref: heroImgRevealRef, visible: heroImgVisible } = useReveal();
    ...
    <>
      <style>{`
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo:hover { transform: translateY(-2px) rotate(-2deg); box-shadow: 0 18px 36px rgba(6,182,212,0.22); }
        .nav-links { display: flex; list-style: none; gap: 0.35rem; flex-wrap: wrap; justify-content: center; }
        .nav-links button {
        ...
        .nav-links button:hover, .nav-links button.active { color: var(--text); background: rgba(37,99,235,0.08); }
        ...
        h2 {
          ...
          margin-bottom: 2.7rem; color: var(--text);
        ...
        .hero h1 {
          font-size: clamp(3rem, 7vw, 5.6rem); line-height: 0.95;
          margin-bottom: 18px; font-weight: 800; letter-spacing: -0.05em;
          background: linear-gradient(90deg, #0f172a, var(--accent-3), var(--accent));
          -webkit-background-clip: text; color: transparent;
        }
        .subtitle {
          font-size: 0.95rem; color: var(--accent-2); margin-bottom: 14px;
        ...
        .cta-button {
          ...
          font-weight: 800; letter-spacing: 0.02em; border: none; cursor: pointer;
          transition: transform 0.3s ease, box-shadow 0.3s ease;
        ...
        .hero-img-wrap { position: relative; display: flex; justify-content: center; perspective: 1200px; }
        .hero-img-wrap::before {
          content: ""; position: absolute; inset: 24px -12px -18px 24px;
          border-radius: 32px;
          background: linear-gradient(135deg, rgba(37,99,235,0.12), rgba(6,182,212,0.12));
          border: 1px solid rgba(15,23,42,0.06);
        }
        .hero-img-wrap img {
          width: min(390px, 100%); height: 500px; object-fit: cover;
          border-radius: 28px; position: relative; z-index: 1;
          box-shadow: var(--shadow); border: 1px solid rgba(15,23,42,0.08);
          transform-style: preserve-3d; transition: transform 0.2s ease;
        }

        /* CARDS (shared glass style) */
        .card {
          background: linear-gradient(160deg, rgba(255,255,255,0.86), rgba(249,251,255,0.96));
          border: 1px solid rgba(15,23,42,0.08);
          box-shadow: var(--shadow); backdrop-filter: blur(12px);
          border-radius: var(--radius);
          transition: transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease;
          opacity: 0; transform: translateY(50px) scale(0.98);
          transition: opacity 0.9s ease, transform 0.9s ease, box-shadow 0.25s ease, border-color 0.25s ease;
        }
        ...
        .about-text-box {
          ...
          box-shadow: 0 12px 30px rgba(15,23,42,0.08); color: var(--muted); line-height: 1.8;
        }
        ...
        .info-item strong {
          color: var(--accent-3); min-width: 120px;
        ...
        .timeline-date {
          background: rgba(37,99,235,0.1); color: var(--accent-3);
          padding: 0.45rem 0.9rem; border-radius: 999px;
          font-size: 0.82rem; margin-bottom: 1rem; display: inline-block; font-weight: 700;
        }
        ...
        .img-placeholder {
          border: 1px dashed rgba(37,99,235,0.26); border-radius: 16px; padding: 1rem;
          text-align: center; background: rgba(248,251,255,0.85); margin-top: 1rem;
          min-height: 180px; display: flex; flex-direction: column;
          align-items: center; justify-content: center; gap: 14px; overflow: hidden;
        }
        .img-placeholder img { width: 100%; height: auto; border-radius: 12px; }
        ...
        .tag {
          background: #eef4ff; color: var(--text); padding: 0.35rem 0.82rem;
          border-radius: 999px; font-size: 0.8rem; border: 1px solid rgba(15,23,42,0.08);
        }
        .open-btn {
          display: inline-block; margin-bottom: 1rem;
          background: linear-gradient(135deg, var(--accent-3), var(--accent-2));
          color: #fff; padding: 0.8rem 1.2rem; border-radius: 12px; border: none;
        ...
        .modal-overlay {
          ...
          justify-content: center; z-index: 3000; padding: 24px;
        }
        .modal-card {
          width: min(1100px, 100%); max-height: 92vh; overflow-y: auto;
          background: linear-gradient(160deg, rgba(255,255,255,0.98), rgba(249,251,255,0.98));
        ...
        .modal-media {
          background: rgba(37,99,235,0.04); border-radius: 18px; overflow: hidden;
          min-height: 220px; border: 1px solid rgba(15,23,42,0.06);
        }
        ...
        .contact-card { padding: 1.8rem; text-align: center; display: block; }
        ...
        @media (max-width: 768px) {
          .nav-links button { padding: 0.35rem 0.55rem; font-size: 0.84rem; }
          .hero-img-wrap img { width: 280px; height: 360px; }
      ...
      <section id="home" className="hero">
        <div className="container hero-grid">
          <div
            ref={heroRevealRef}
            className={`reveal ${heroRevealVisible ? "visible" : ""}`}
          >
            <h1>Oussama REBAI</h1>
            <p className="subtitle">Élève ingénieur en génie mécanique & design industriel durable</p>
            <p className="description">
              Étudiant ingénieur en double diplôme à l'EPF, je recherche une alternance à partir de
              septembre dans le domaine industriel. Intéressé par l'usinage, le contrôle qualité et
              la conception mécanique, je suis polyvalent, rigoureux et motivé à mettre en pratique
              mes compétences en fabrication, métrologie et conception, tout en contribuant à
              l'amélioration des processus et à la conformité des pièces.
            </p>
            <button className="cta-button" onClick={() => scrollTo("contact")}>Me contacter</button>
          </div>

          <div
            ref={heroImgRevealRef}
            className={`hero-img-wrap reveal ${heroImgVisible ? "visible" : ""}`}
          >
            <div ref={heroTilt} style={{ display: "contents" }}>
              <Image
                src="/images/oussema pdp.jpeg"
                alt="Oussama REBAI"
                width={390}
                height={500}
      ...
      <section id="about">
        <div className="container">
          ...
          <RevealTiltCard className="about-info">
            {[
              ["Localisation :", "Mobilité sur toute la France"],
              ["Formation :", "EPF France & IIT Sfax"],
              ["Domaine :", "Génie mécanique / Design industriel durable"],
              ["Email :", "oussama.rebai@epfedu.fr"],
              ["Téléphone :", "+33 7 53 90 23 28"],
              ["Objectif :", "Évoluer entre ingénierie, innovation et management technique"],
            ].map(([k, v]) => (
            ...
            {projects.map((p) => (
              <RevealTiltCard key={p.title} className="project-card">
                <div className="project-img">
                  <Image src={p.image} alt={p.title} width={600} height={230} style={{ objectFit: "cover", borderRadius: 14 }} />
                </div>
                <div className="project-body">
                  <div className="project-title">{p.title}</div>
                  <div className="project-desc">{p.description}</div>
                  {p.hasDetail && (
                    <button className="open-btn" onClick={() => setDetailOpen(true)}>
                      Voir le projet réalisé
                    </button>
                  )}
        ...
        <div className="modal-overlay" onClick={() => setDetailOpen(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="modal-desc">
              Cette page présente le projet réalisé : étude fonctionnelle, calculs de
              dimensionnement, modélisation 3D, simulation, fabrication, assemblage et validation
              expérimentale. Remplace les médias ci-dessous par les vraies photos et la vidéo de ton projet.
            </p>
            <div className="modal-gallery">
              {["/images/projet-meca.jpg", "/images/projet-meca-2.jpg", "/images/projet-meca-3.jpg"].map((src, i) => (
                <div key={i} className="modal-media">
                  <Image src={src} alt={`Projet mécanique ${i + 1}`} width={400} height={300} style={{ objectFit: "cover" }} />
                </div>
              ))}
              <div className="modal-media">
                <video controls style={{ width: "100%", height: "100%" }}>
                  <source src="/images/projet-meca.mp4" type="video/mp4" />
      ...
      <footer>
        <div className="container">
          <p>&copy; 2026 Oussama REBAI. Tous droits réservés.</p>
        </div>
      </footer>
    </>
  );
